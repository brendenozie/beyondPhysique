      ...
      <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {exercises.map((exercise: any) => (
          <Link href={`/exercises/${exercise.id}`} key={exercise.id}>
            <div className="relative bg-gradient-to-br from-white to-gray-100 rounded-lg p-6 shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 hover:scale-105">
              {/* Decorative background accent */}
              <div className="absolute inset-0 opacity-10 bg-gradient-to-r from-blue-400 to-green-400 rounded-lg"></div>

              <h2 className="text-2xl font-semibold text-gray-900 mb-3 relative z-10 transition-transform duration-300 hover:scale-110">
                {exercise.exName}
              </h2>
