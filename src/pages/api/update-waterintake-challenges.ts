...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const runningChallenges = await prisma.waterIntakeChallenges.findMany({
            where: {
                releaseDate: {
        ...
        if (runningChallenges.length >= 5) {
            return res.status(200).json({
                message: 'There are already 5 running water intake challenges. No new challenges released.',
                runningChallengesCount: runningChallenges.length
            });
        }

        // Step 1: Prioritize challenges with no release date
        let challengesToRelease = await prisma.waterIntakeChallenges.findMany({
            where: {
        ...
        if (challengesToRelease.length < 5) {
            const additionalChallenges = await prisma.waterIntakeChallenges.findMany({
                where: {
                    endDate: {
        ...
        for (const challenge of challengesToRelease) {
            await prisma.waterIntakeChallenges.update({
        ...
        console.error('Error updating water intake challenges:', error);
